...
- `activation` is an optional extension for **orchestration evals**, which check that the [Cross-Cutting Concern Triage](../skills/using-agent-skills/SKILL.md#cross-cutting-concern-triage) activates the right skills and *only* those. It holds three lists of skill names: `activate` (must be activated), `not_activate` (irrelevant, so they must stay inactive; at least one entry), and `optional` (judgment calls either way). In CI, the runner checks that the names are real skills, that no name appears in two lists, and that every concern skill in the triage table is classified. Adding a triage question therefore forces every orchestration eval to take a position on the new skill. Under `--behavioral`, the runner reads the `TRIAGE.md` the agent wrote and parses its `ACTIVATED:` line. The eval fails if a required skill is missing or an irrelevant one appears, whatever the grader says. The check result is saved as `activation_check` in `grading.json`. Activation evals must be `execution` evals whose prompt asks for `TRIAGE.md` with an `ACTIVATED:` line. See evals 3–12 in [`using-agent-skills.json`](cases/using-agent-skills.json), which run against the multi-tenant full-stack `orchestration-projects-app` fixture and the single-tenant, backend-only `orchestration-invoicing-api` fixture.
