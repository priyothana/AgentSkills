...
2. Select the task's cross-cutting skills: invoke exactly the skills in its Skills field. If the task has no Skills field (no plan, or a plan written before triage), first run the cross-cutting concern triage from the agent-skills:using-agent-skills skill for this task and record the result. Do not load concern skills the triage did not activate
...
3. **Plan if needed.** If there is no `tasks/plan.md`, invoke agent-skills:planning-and-task-breakdown to generate one. If an existing plan has no Cross-Cutting Concerns section or its tasks have no Skills field, run the cross-cutting concern triage from the agent-skills:using-agent-skills skill now and add both before the checkpoint, so the approval covers which skills each task activates.
