...
| [multi-tenant-architecture](skills/multi-tenant-architecture/SKILL.md) | Detects whether an app is multi-tenant and which tenancy model it uses, then keeps tenant context and isolation intact across APIs, data access, caches, jobs, events, storage, migrations, and tests | Changing code in an application that serves multiple tenants, organizations, or workspaces |
| [backend-application-architecture](skills/backend-application-architecture/SKILL.md) | Route, handler, service, and repository layers with inward dependencies, one place for error mapping, and service-owned transactions, in any backend stack | Adding or restructuring backend endpoints, services, or data access |
| [application-rbac-and-authorization](skills/application-rbac-and-authorization/SKILL.md) | Permission matrix of roles, resources, and actions, one decision function enforced server-side, deny by default, escalation checks, table-driven role tests | Adding roles, permissions, admin actions, or guarded features |
| [database-seeding-and-data-bootstrap](skills/database-seeding-and-data-bootstrap/SKILL.md) | Classifies initial data, orders seeds by dependency, makes them idempotent, and fences destructive steps behind environment guards | Writing seeds, fixtures, reference data, or first-run setup |
| [frontend-localization-and-i18n](skills/frontend-localization-and-i18n/SKILL.md) | Translation keys, locale detection and fallback, plural rules, Intl formatting, time zones, RTL layout, localization tests | Adding languages or formatting dates, numbers, and prices for a locale |
| [frontend-routing-and-navigation](skills/frontend-routing-and-navigation/SKILL.md) | Route structure, validated params, guards that never flash protected content, route-level loading, 404 and error routes, deep links, history, unsaved-change prompts | Adding pages, URLs, protected routes, or navigation behavior |
| [frontend-state-and-lifecycle-patterns](skills/frontend-state-and-lifecycle-patterns/SKILL.md) | One owner per value, derived over stored state, effects with complete dependencies and cleanup, stale-response protection, async UI states, mapped across React, Vue, Angular, and Svelte | Writing effects, stores, data fetching, or forms, or fixing races and leaks |
...
| [database-query-and-script-management](skills/database-query-and-script-management/SKILL.md) | Scope, preview, back up, and apply data changes in a transaction with a row-count guard, dry-run by default, with rollback and an audit trail | Running data fixes, backfills, or bulk updates and deletes on real databases |
